...
void child_handler(Dataset *training, Dataset *testing, int K, 
    ...
    //TODO error check and exit properly
    int start_idx = 0, N = 0;
    ...
    for (int i = start_idx; i < start_idx + N; i++) {
        int ans = knn_predict(training, &(testing->images[i]), K, fptr);
        if (ans == testing->labels[i]) {
            num_correct++;
        }
    }
   
    if (write(p_out, &num_correct, sizeof(int)) != sizeof(int)) {
        perror("write");
        exit(1);
    }
...
double distance_cosine(Image *a, Image *b){
    double ab_sum = 0, a_sq_sum = 0, b_sq_sum = 0;

    for (int i = 0; i < NUM_PIXELS; i++) {
        ab_sum += a->data[i] * b->data[i];
        a_sq_sum += a->data[i] * a->data[i];
        b_sq_sum += b->data[i] * b->data[i];
    }

    return 2*acos((ab_sum) / (sqrt(a_sq_sum) * sqrt(b_sq_sum))) / M_PI;
}
