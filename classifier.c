...
int main(int argc, char *argv[]) {
    ...
    double (*fptr)(Image *, Image *) = &distance_euclidean;
     if (strncmp(dist_metric, "euclidean", strlen(dist_metric)) == 0) {
    ...
    if (num_procs < 1) {
        fprintf(stderr, "The number of processes cannot be less than one.\n");
        exit(1);
    }

    if (num_procs > testing->num_items) {
        fprintf(stderr, "The number of processes cannot exceed the test count.\n");
        exit(1);
    }
    ...
    int par_to_child_fd[num_procs][2];
    int child_to_par_fd[num_procs][2]; 

    for (int i = 0; i < num_procs; i++) {
        // Call pipe before forking
        if (pipe(par_to_child_fd[i]) == -1 || pipe(child_to_par_fd[i]) == -1) {
            perror("pipe");
            exit(1);
        }
        ...
        if (r < 0) {
            ...
            // Close the read end of child writing pipe
            if (close(child_to_par_fd[i][0]) == -1) {
                perror("close");
                exit(1);
            }

            // Close write end of parent writing pipe
            if (close(par_to_child_fd[i][1]) == -1) {
                perror("close");
                exit(1);
            }
            
            // Before forking, the parent had open the read end
            // to all previously forked children
            int child_no;
            for (child_no = 0; child_no < i; child_no++) {
                if (close(child_to_par_fd[child_no][0]) == -1) {
                    perror("close");
                    exit(1);
                }
            }

            child_handler(training, testing, K, fptr, par_to_child_fd[i][0], 
                    child_to_par_fd[i][1]);

            // Free datasets in child before exiting
            ...
            // Distribute the work to the children by writing their starting 
            // index and write starting index and number of images to process 
            // for child.

           // Better chunkifying for evenly sized chunks

            int chunk_size = testing->num_items / num_procs;
            int chunk_rem = testing->num_items % num_procs;
            ...
            // Close the read end of parent writing pipe
            if (close(par_to_child_fd[i][0]) == -1) {
                perror("close");
                exit(1);
            }

            // Close the write end of child writing pipe
            if (close(child_to_par_fd[i][1]) == -1) {
                perror("close");
                exit(1);
            }

            // Write <start_idx> and <N> to write end of the parent writing pipe 

            if (write(par_to_child_fd[i][1], &start_idx, sizeof(int)) != sizeof(int)) {
                perror("write");
                exit(1);
            }

            if (write(par_to_child_fd[i][1], &N, sizeof(int)) != sizeof(int)) {
                perror("write");
                exit(1);
            }

            // Close the write end of the parent writing pipe
            if (close(par_to_child_fd[i][1]) == -1) {
                perror("close");
                exit(1);
            }
    ...
    // Wait for all <num_procs> child processes to end
    int status;
    for (int i = 0; i < num_procs; i++) {
        if (wait(&status) == -1) {
            perror("wait");
    ...
    int child_ans = 0;
    for (int i = 0; i < num_procs; i++) {
        if (read(child_to_par_fd[i][0], &child_ans, sizeof(int)) != sizeof(int)) {
            perror("read");
            exit(1);
        }
    ...
    // Close read end pipes of child write pipe
    for (int i = 0; i < num_procs; i++) {
        if (close(child_to_par_fd[i][0]) == -1) {
            perror("close");
            exit(1);
